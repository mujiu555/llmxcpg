...
class DatasetProcessor:
    ...
    def process_dataset(self):
        ...
        try:
            ...
            self.llm_manager = LLMManager(
                model_type, self.llm_model_name, port=self.llm_port
            )
            ...
            for i, sample in enumerate(self.dataset_slice):
                ...
                try:
                    ...
                    if result:
                        active_joern_project = (
                            file_name  # Mark project as potentially loaded
                        )
                        self._write_processed_sample(result)
                        self._log_sample_message(
                            logging.INFO, f"Successfully processed and saved."
                        )
                    else:
                        # Error occurred within _process_single_sample, logs already added
                        self._log_sample_message(
                            logging.WARNING, f"Processing failed. Check logs."
                        )
                    ...
                    self._log_sample_message(
                        logging.ERROR,
                        f"Unhandled error processing sample: {e}",
                        exc_info=True,
                    )
                    ...
                    if active_joern_project:
                        try:
                            self._log_sample_message(
                                logging.DEBUG,
                                f"Cleaning up Joern project: {active_joern_project}",
                            )
                            self.joern_manager.delete_project(active_joern_project)
                        except Exception as cleanup_e:
                            self._log_sample_message(
            ...
            logging.error(
                f"[Port {self.port}] Critical error in thread execution: {e}",
                exc_info=True,
            )
    ...
    def _process_single_sample(self, sample: Dict, file_name: str) -> Dict | None:
        ...
        try:
            ...
            self._log_sample_message(
                logging.DEBUG, "Generating CPGQL queries using LLM."
            )
            ...
            if (
                not llm_answer
                or "queries" not in llm_answer
                or not isinstance(llm_answer["queries"], list)
            ):
                self._log_sample_message(
                    logging.WARNING,
                    f"Failed to extract valid list of queries from LLM response. Response text: {completion_text}",
                )
                raise Exception(
                    "Failed to extract valid queries list from LLM response."
                )

            generated_queries = llm_answer["queries"]
            self._log_sample_message(
                logging.INFO, f"LLM generated {len(generated_queries)} queries."
            )
            self._log_sample_message(
                logging.DEBUG, f"Generated Queries: {generated_queries}"
            )

            # --- Run queries and extract paths ---
            self._log_sample_message(
                logging.DEBUG, "Running generated queries in Joern."
            )
            successful, paths = self.joern_manager.run_queries(
                generated_queries, code_content
            )

            self._log_sample_message(
                logging.INFO, f"Joern query validation: successful={successful}"
            )
        ...
        try:
            ...
            try:
                ...
                if not isinstance(existing_data, list):
                    self._log_sample_message(
                        logging.WARNING,
                        f"Output file {self.output_file} does not contain a JSON list. Overwriting.",
                    )
                    existing_data = []
            ...
            logging.info(
                f"[Port {self.port}] Appended result for sample {self.current_sample_uuid} to {self.output_file}"
            )

        except Exception as e:
            logging.error(
                f"[Port {self.port} | Sample {self.current_sample_uuid}] Error writing processed sample to {self.output_file}: {e}",
                exc_info=True,
            )
            self.sample_log_buffer.append(
                f"[ERROR] Failed to write result to output file: {e}"
            )
        ...
        try:
            ...
            try:
                ...
                if not isinstance(existing_logs, list):
                    logging.warning(
                        f"Log file {self.logs_file} corrupt. Starting new list."
                    )
                    existing_logs = []
            except (FileNotFoundError, json.JSONDecodeError):
                existing_logs = []
            ...
            logging.info(
                f"[Port {self.port}] Appended logs for sample {self.current_sample_uuid} to {self.logs_file}"
            )

        except Exception as e:
            logging.critical(
                f"[Port {self.port} | Sample {self.current_sample_uuid}] CRITICAL ERROR: Failed to write to error log file {self.logs_file}: {e}",
                exc_info=True,
            )
        finally:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Process a dataset of code samples using Joern and an LLM in parallel."
    )
    ...
    parser.add_argument(
        "--llm-model-type",
        type=str,
        choices=["vLLM", "DeepSeek"],
        default="vLLM",  # Default based on original code
    ...
    parser.add_argument(
        "--llm-port", type=int, default=9001, help="Port number for the LLM service."
    )
    ...
    try:
        ...
        logging.info(
            f"Loaded dataset with {len(dataset)} samples from {args.dataset_file}"
        )
    ...
    current_slice_size = slice_size + (1 if 1 < remainder else 0)
    end_idx = start_idx + current_slice_size

    dataset_slice = dataset[start_idx:end_idx]

    output_file = os.path.join(results_dir, f"thread_{1 + 1}_results.json")
    logs_file = os.path.join(logs_dir, f"thread_{1 + 1}_logs.json")
    port = args.base_joern_port + 1
