...
class DatasetProcessor:
    ...
    def process_dataset(self):
        ...
        if not self.joern_manager.check_server_health():
            self._log_sample_message(logging.INFO, "Joern server not running. Starting it...")
            if not self.joern_manager.recreate_server():
                self._log_sample_message(logging.ERROR, "Failed to start Joern server. Exiting thread.")
...
def main():
    ...
    parser.add_argument("-j", "--joern-path", type=str, required=True,
